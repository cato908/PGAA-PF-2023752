	...
	case WM_INITDIALOG: {
		...
		if (miInfo != nullptr) {
			...
			else
			{
				...
				HBITMAP ALL; //1
				ALL = (HBITMAP)LoadImage(NULL, miInfo->foto, IMAGE_BITMAP, 130, 150, LR_LOADFROMFILE); //2
				SendDlgItemMessage(hwnd, ID_STAPICIDU, STM_SETIMAGE, IMAGE_BITMAP, (LPARAM)ALL); //3 
...
void eliminarpromo(char nompromocion[50]) {
	...
	if (proaux == nullptr)
	{
		...
		if (proaux == nullptr) {
			...
			else { //Si es cualquier nodo que no sea el inicio
